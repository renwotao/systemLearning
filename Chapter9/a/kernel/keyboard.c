...
/*
	kb_wait
*/
PRIVATE void kb_wait() // 等待 8242 的输入缓冲区
{
	u8 kb_stat;
	do
	{
...
PUBLIC void init_keyboard()
{
	...
	shift_l = shift_r = 0; // warning: 如果未初始化则输入都为shift + x
	alt_l = alt_r = 0;
	ctrl_l = ctrl_r = 0;	
...
PUBLIC void keyboard_read(TTY* p_tty)
{
	...
	u32 key = 0; /* 用一个整型来表示一个键。比如，如果Home 被
			按下，则 key 值将为定义在keyboard.h 中的‘HOME’*/
...
{
	u8 scan_code;
	while (kb_in.count <= 0) {} // 等待下一个字节的输入
